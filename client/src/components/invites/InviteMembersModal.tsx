...
const InviteMembersModal: React.FC<InviteMembersModalProps> = ({
  ...
  // Get current template with placeholders replaced
  // Note: This is just for preview - actual invitation links are generated by the backend
  // using CLIENT_URL environment variable (https://www.g8road.com in production)
  const getCurrentTemplate = () => {
    if (!templates) return '';
    
    const template = method === 'email' ? templates.inviteTemplateEmail : templates.inviteTemplateSMS;
    
    // Use a placeholder that represents the actual production URL format
    // The backend will generate the real link with CLIENT_URL
    const exampleLink = window.location.hostname === 'localhost'
